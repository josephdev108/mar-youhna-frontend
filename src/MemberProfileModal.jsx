...
export default function MemberProfileModal({ memberId, onClose, Modal, Icon }) {
    ...
    <Modal wide title={member?.name ?? 'الملف الشخصي'} onClose={onClose}>
      {loading ? (
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3 rounded-xl bg-gradient-to-l from-blue-600 to-indigo-700 p-4 text-white">
            ...
            <div className="min-w-0">
              ...
              {member.phone && (
                <a href={`tel:${member.phone}`} className="mt-0.5 block font-mono text-sm text-blue-100" dir="ltr" onClick={(e) => e.stopPropagation()}>
                  {member.phone}
                </a>
              )}
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2 text-center text-xs">
                <div className="rounded-lg bg-blue-50 py-2">
                  <p className="text-lg font-bold text-blue-700">{summary.total_lectures}</p>
                  <p className="text-blue-600">محاضرات</p>
                </div>
